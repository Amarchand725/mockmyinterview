        ...
        <div class="py-3">
            ...
            <div class="row py-3">
                @foreach ($slots['morning_slots'] as $key=>$slot)
                    <div class="col-md-3">
                        <button type="button" class="mt-3 slot m-slot-btn" data-index="{{ $key }}" value="{{ $slot }}">{{ $slot }}</button>
                        <input type="hidden" id="slot-m-{{ $key }}" name="mornings[]">
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class=" py-3">
            ...
            <div class="row py-3">
                @foreach ($slots['evening_slots'] as $key=>$slot)
                    <div class="col-md-3">
                        <button type="button" class="mt-3 slot e-slot-btn" data-index="{{ $key }}" value="{{ $slot }}">{{ $slot }}</button>
                        <input type="hidden" id="slot-e-{{ $key }}" name="evenings[]">
                    </div>
                @endforeach
            </div>
        </div>
