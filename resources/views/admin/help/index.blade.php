...
@section('content')
<input type="hidden" id="page_url" value="{{ route('help.index') }}">
							...
							@foreach($models as $key=>$model)
								<tr id="id-{{ $model->slug }}">
									...
									<td>
										...
										@endcan
										@can('help-delete')
                                            <button class="btn btn-danger btn-xs delete" data-slug="{{ $model->slug }}" data-del-url="{{ url('help', $model->slug) }}"><i class="fa fa-trash"></i> Delete</button>
										@endcan
									</td>
								</tr>
							@endforeach
